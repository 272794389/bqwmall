...
Page({
  ...
  */
  ...
  navRecharge:function(e){
     this.setData({
       ...
       paid_price: this.data.picList[0].price,
       numberPic: this.data.picList[0].quota,
       activePic: 0
     })
  },
   /**  
  ...
  */
  picCharge:function(e){
    if (e.currentTarget.dataset.id == this.data.picList.length) {
      this.setData({
        activePic: e.currentTarget.dataset.id,
        paid_price: '',
        numberPic: ''
      })
    }else {
      this.setData({
        number: '',
        activePic: e.currentTarget.dataset.id,
        paid_price: e.currentTarget.dataset.pic,
  ...
  getRecharge: function () {
    ...
    getRechargeApi().then(res => {
      that.setData({ 
        ...
        paid_price: res.data.recharge_quota[0].price,
        numberPic: res.data.recharge_quota[0].quota,
  ...
  submitSub:function(e){
    ...
    if (that.data.active){
      ...
      if (value){
        ...
        price = that.data.numberPic;
        paid_price = that.data.paid_price;
      }
      wx.showLoading({
        title: '正在支付',
      })
      rechargeRoutine({ price: price, type: 0, paid_price: paid_price}).then(res=>{
        wx.hideLoading();
