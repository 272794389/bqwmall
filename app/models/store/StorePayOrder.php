...
class StorePayOrder extends BaseModel
{
    ...
    {
        return WechatService::paymentPrepare(null,$orderInfo['order_id'],$orderInfo['price'],'user_vip','购买vip', '', 'MWEB');
    }
    ...
    {
        return WechatService::jsPay(WechatUser::uidToOpenid($orderInfo['uid'], 'openid'),$orderInfo['order_id'],$orderInfo['price'],'user_vip','购买vip');
    }
    ...
    public static function paySuccess($orderId, $paytype = 'weixin')
    {
        ...
        if($agentAmount>0){
            ...
            if($districtInfo['agent_uid']>0){//地区代理佣金
                ...
                if($res&&$districtAmount>0){
                    $res = StorePayLog::expend($districtInfo['agent_uid'], $orderInfo['id'], 0, $use_amount, 0, 0, 0,$repeat_point,$fee, '地区代理商奖励');
                }
            }
            if($cityInfo['agent_uid']>0){//城市代理佣金
                ...
                if($res&&$cityAmount){
                    $res = StorePayLog::expend($cityInfo['agent_uid'], $orderInfo['id'], 0, $use_amount, 0, 0, 0,$repeat_point,$fee, '城市代理商奖励');
                }
            }
            if($province['agent_uid']>0){//省级代理佣金
                ...
                if($res&&$agentAmount>0){
                    $res = StorePayLog::expend($province['agent_uid'], $orderInfo['id'], 0, $use_amount, 0, 0, 0,$repeat_point,$fee, '省级代理商奖励');
                }
            }
        }
        ...
        if($agentAmount>0){
            ...
            if($districtInfo['inspect_uid']>0){//地区总监佣金
                ...
                if($res&&$districtAmount>0){
                    $res = StorePayLog::expend($districtInfo['inspect_uid'], $orderInfo['id'], 0, $use_amount, 0, 0, 0,$repeat_point,$fee, '地区代理商奖励');
                }
            }
            if($cityInfo['inspect_uid']>0){//城市总监佣金
                ...
                if($res&&$cityAmount){
                    $res = StorePayLog::expend($cityInfo['inspect_uid'], $orderInfo['id'], 0, $use_amount, 0, 0, 0,$repeat_point,$fee, '城市代理商奖励');
                }
            }
            if($province['inspect_uid']>0){//省级总监佣金
                ...
                if($res&&$agentAmount>0){
                    $res = StorePayLog::expend($province['inspect_uid'], $orderInfo['id'], 0, $use_amount, 0, 0, 0,$repeat_point,$fee, '省级代理商奖励');
                }
            }
        }
