  ...
  <div class="my-account">
    <div class="wrapper">
      ...
      <div class="nav acea-row row-middle">
        <router-link class="item" :to="'/user/re_pay/0'">
          <div class="pictrue"><img src="@assets/images/record1.png" /></div>
          <div>全部记录</div>
        </router-link>
        <router-link class="item" :to="'/user/re_pay/1'">
          <div class="pictrue"><img src="@assets/images/record2.png" /></div>
          <div>支出记录</div>
        </router-link>
        <router-link class="item" :to="'/user/re_pay/2'">
...
export default {
  ...
  methods: {
    getIndex: function() {
      ...
      getBalance().then(
        res => {
          that.now_money = res.data.pay_point;
          that.in_amount = res.data.in_paypoint;
          that.out_amount = res.data.out_paypoint;
        },
        err => {
